...
// Shareable customer link for an individual reservation portal (with PIN built
// in), plus a one-click "Welcome to the Cruise Experience Center" email.
export default function CopyReservationLink({
  token, sailDate, name, email,
}: { token: string; sailDate: string; name?: string; email?: string }) {
  ...
  async function sendEmail() {
    ...
    try {
      const res = await fetch("/api/send-portal-link", {
        ...
        body: JSON.stringify({ to, name, link: buildUrl(), pin, kind: "reservation" }),
      });
