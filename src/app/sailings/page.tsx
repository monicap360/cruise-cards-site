...
export default async function SailingsPage({
  ...
  const { ship, line, year, month } = await searchParams;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {blocks.map((b) => {
              ...
              return (
                ...
                >
                  <div className="label-mono text-[10px] uppercase text-sky-400/70 mb-2">
                    {b.cruiseLine}
                  </div>
                  ...
                  <div className="text-white/45 text-sm mb-3">
                    {b.nights} nights · returns {fmtDate(b.returnDate)}
                  </div>
                  <p className="text-white/55 text-sm flex-1">{b.itinerary}</p>
